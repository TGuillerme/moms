...
Both organisms and the environments in which they evolve are a combination of a multitude of traits whether phenotypic, phylogenetic, chemical, physiological, behavioural etc.
Since the last 20 years, an emerging consensus is that these traits should be analysed all together into *trait-spaces* rather than in isolation [e.g. @donohue2013, @hopkins2017 ].
This has led to and explosion of trait space approaches facilitated by increased computational power and statistical methods [SomeBigDataCite] as well as by the implementation of statistical methods in R packages [@oksanen2007vegan, @adams2013geomorph, @blonder2018, @disprity].
...
For example, in palaeobiology, @wright2017 looked at blablabla;
in evolutionary biology @marcy2016 looked at blabalbalbal;
in ecology @jones2015 looked at blalbla.
-> In macroevolution it's often refered as disparity or morphospace analysis
-> In ecology it can be dissimilarity analysis, functional niche, etc..
-> In math, it's just the same mathematical objects with each column representing a trait (or a transformation thereof) and each row representing an observation (taxon, field site, etc.) [@disprity].

<!-- All the fields differ, the methods remain really similar -->
One further similarity of this trait-space approach are what researchers are doing with the trait-space.
In all fields they look at the occupancy of observations in this trait space [e.g. using disparity metrics @wills2001, @hopkins2017, @disprity in evolution; or hypervolume approaches @blonder2018,@mammola2019 in ecology].
However, surprisingly few work has been published on the behaviour of these metrics of multidimensional space occupancy [but see @ciampaglio2001, @mammola2019].
<!-- This is entering the danger zone! -->
In fact in many papers, people have been using the default method without thought on what to capture.
Often, a default space occupancy metric is used regardless of the question used.
For example, if the question in 
-> using the wrong metric for the wrong question



However, the specific justification of using each tool is often disregarded or dismissed
    -> What is the question? What to measure?
Here we propose a novel classification of occupancy metrics and look at what they pick up
-> In simulations
-> In empirical data
