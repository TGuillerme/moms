...
Multidimensional analysis of traits are now a common tool-kit in ecology and evolution.
Such analysis are based on multidimensional datasets (in which each dimension represents a trait) that summarise all observed and potential trait combinations in a multidimensional trait-space (e.g. a morphospace or an ecospace).
...
By using these metrics, researchers have investigated how space occupancy changes through time, in relation to other groups of organisms, and in response to global environmental changes (such as global warming events or mass extinctions).

In macroevolutionary and ecological studies these metrics are often referred to as respectively disparity or dissimilarity metrics and can be generalised as space occupancy metrics.
Although there are a few (well defined and well studied) space occupancy metrics that are used in ecology and evolution, we lack a general description of the majority of those metrics, especially in macroevolution.
...
For example, does a reduction in volume in the trait-space correspond to episodes of extinction or changes in traits distribution? Does an overlap in trait-space corresponds to an overlap in ecological niches?

Here we propose a broad classification of these space occupancy metrics into three categories: metrics that capture changes in volume, density or position of the trait-space.
We analyse the behaviour of height space occupancy metrics (including two new ones focusing changes in density and position) on a series of simulated and empirical datasets.
We found that there are no one-size-fits-all metric but that some metrics are better at capturing specific aspects of the trait-space.
Furthermore we propose [`moms`](@@@), tool with a user friendly graphical interface that allows visualisation of numerical changes of diverse metrics following space transformations (e.g. contraction, displacement, etc.) and could help researchers with choosing the right space occupancy metrics knowing the properties of their trait-space and the biological question at hand.
...
<!-- TG: Narrow down the figure to just A D and I -->
...
<!-- TG: also mention that we do that for 25 metrics in total in the supplementary materials -->
...
<!-- TG: add the thingy to space change figure and shorten this fable caption -->
...
<!-- TG: add somewhere "Some metrics can capture different aspects of trait-space occupancy at the same time: the average distance from centroid (vol) is correlated to the sum of variance (vol) but not to the ellipsoid volume (vol) but still to the average nearest neigbhour distance (density)."-->
...
# Conclusion
<!-- TG: Maybe we don't need this header -->
