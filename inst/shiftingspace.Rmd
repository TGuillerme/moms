...
author: "Thomas Guillerme, Mark Puttick, Ariel E Marcy, Vera Weisbecker"
...
<!-- Alternative title:
A broad classification of metrics for summarising multidimensional spaces occupancy.
Shifting spaces: which metrics best summarise occupancy in multidimensional spaces?
Shifting spaces: choosing metrics to summarise multidimensional spaces occupancy
Introducing MOMS: a user-friendly visualization tool to choose the best metrics for measuring multidimensional data
-->
...
Taxa 
<!-- AM: Just being pedantic – is it always taxa in ecology? Are there any studies on say, traits of an ecosystem or something like that? -->
will typically only occupy a subset of this space, so researchers apply one or more metrics to quantify the way in which organisms “inhabit” that trait-space.
Researchers use these metrics to investigate how space occupancy changes through time, in relation to other groups of organisms, and in response to global environmental changes, such as global warming
<!-- AM: This term always seems a little outdated compared to “climate change” these days, but maybe you’re also referring to really ancient warming as well?  -->
 events or mass extinctions.

In macroevolutionary and ecological studies these metrics are often referred to as disparity or dissimilarity metrics, respectively, and can be generalised as “space occupancy metrics”
<!-- AM: You’re coining this phrase right? Take credit   -->
, a term that is applicable to both disciplines.
<!-- AM: Or something like this, I thought this was an important point that should be emphasized in the abstract -->
Although there are a few well-defined and well-studied space occupancy metrics that are used in ecology and evolution, we lack a general description of the majority of those metrics
<!-- AM: This was confusing to me – I tried a slightly different sentence structure below, see if you like it.  -->
, especially in macroevolution.
<!--
 AM: include:
Besides a small number of well-defined and well-studied space occupancy metrics, the majority of commonly-used metrics in ecology and evolution lack general descriptions, especially in macroevolution.
 MP: include:
...
-->
Therefore the mathematical and biological meaning of such space occupancy metrics can be vague.
This has consequences when interpreting
<!-- AM: Maybe be more explicit / tangible here? 
Or combine with last sentence? E.g …. “be vague, which can make it difficult to interpret space occupancy changes and link them to biological phenomena.”  -->
changes in space occupancy and linking them to biological phenomena.
...
Here we introduced [`moms`](@@@), a user-friendly tool based on a graphical interface that allows users to compare and visualisation of numerical changes with a range  metrics.
<!-- AM: My understanding is that this sentence is meant to convey that moms can both 1) visualize plot changes following a space transformation and 2) track the corresponding change(s) to the value of a metric or metrics. Is that right? If so, I think it would be really helpful to say this more clearly since this is the only sentence you’re using in the abstract to sell would-be moms users that this is the tool for them and it’s not too hard to use.   -->
Users are also provided with tools to transform their data in space (e.g. contraction, displacement, etc.).
These visualizations are designed to help researchers choose the right space occupancy metrics, given the properties of their trait-space and their biological question.


# Introduction

Groups of species and environments share specific, easily recognisable, correlated characteristics (e.g. guilds or biomes) of many kinds (
<!-- MP: I would try to minimize the use of parentheses/brackets throughout the MS as there are a lot at present. -->
e.g. phenotypic, physiological, phylogenetic, behavioural).
...
Ordination techniques (or dimensionality reduction) are a popular way to analyse sets of multiple traits and create a trait-space of the study
<!-- AM: More professional verb here? -->
 system [see @legendre2012 for a summary].
...
<!-- MP: I’ve re-structured this so please check it still makes sense. -->
...
<!-- TG: check vocab from Tucker et al 2018 -->
...
This is
<!-- AM: More specific here? I think it works okay as is but I forgot what “this” referred to since all the questions were interesting -->
 achieved in various ways such as using disparity metrics (or indices)
<!-- MP: I would choose metric or index and just use it throughout without the parentheses containing the other name. Both are shown in the table so readers can always refer back to that in the unlikely event of confusion. -->
  in macroevolution [@wills2001; @hopkins2017; @disprity] or comparing hypervolumes in ecology [@donohue2013; @diaz2016; @blonder2018; @mammola2019].
...
For example, any analysis of volume occupancy through trait space may remain constant, and this will lead to supporting a certain biological conclusion. Yet, by analyzing an alternative aspect of multidimensional occupancy, may indicate the position is change while the volume is constant; this would likely lead to a different biological conclusion. 
...
By only applying a small number or even a single measure of multidimensional occupancy, workers may be limiting the
<!-- AM: Since you’re also talking about metrics meant to measure real-world diversity, this might be confusing phrasing. 
Could you mean, using metrics that only measure one aspect of the multidimensional trait space may restrain… ?
 -->
potential of multidimensional analysis [@villéger2008].
...
Unsurprisingly, we found no one metric describes all changes through a morphospace and the results from each metric are dependent on the characteristics of the space and the hypotheses.
However, further research of the different metrics analysed here can help understanding
<!-- AM: Second use of understanding. Predict? Or change the 1st one to familiarity. I think this sentence could be clearer and resolving these two words will help. 
I think this is one of the places where moms could be set up as the tool that helps researchers choose a metric by giving them experience with how those metrics get warped by particular space transformations that may or may not happen to their data / would be needed to test their question.  
 -->
 their “null” behavior (i.e. how does occupancy change with random/non-random changes to the trait-space).
...
For the purposes of this paper, we will call trait-spaces
<!-- AM: Maybe make it more authoritative sounding? Like: 
In this paper, we define trait-spaces as…
 -->
 any matrix where rows are observations and columns are traits.
These traits can widely vary in number and types.
For example, traits could be coded as discrete [e.g. presence of absence of a bone; @beck2014; @wright2017], continuous measurements [e.g. leaf area; @diaz2016] or more sophisticated measures of 2- or 3-dimensions [e.g. landmark position; @marcy2016].
...
This aspect of space occupancy is used routinely in macroevolution and ecology [@wills2001; @blonder2018] because disparity or dissimilarity analysis often look at the “spread” of observations in multidimensional space.
...
Density is less commonly measured compared to morphospace volume, but it is still used in both evolution [e.g. the average pairwise distance; @geiger2008] and ecology
<!-- AM: Super small point – usually you report what ecology does first – switch for consistency?  -->
 [e.g. the minimum spanning tree length; @oksanen2007vegan].
...
<!-- MP: This feels a bit out of place here and may come across as overly didactic. Maybe it could be removed?
 -->
For example, if group A has a greater volume than group B but both have the same density, similar mechanisms could be driving both groups’ trait-space occupancy (e.g. a Brownian motion).
<!-- MP: I understand your point here, but I worry it may be a term too far for readers unfamiliar with the subject. Could BM be removed? -->
...
[`moms`](@@@) will allow resarchers to comprehensively assess the properties of their trait-space and the metrics associated with their specific biological question.
...
We also introduce one new metric (the average displacements) which we expect to be only influenced by trait space position change, which to our knowledge is not available in the literature.
...
Furthermore, the [`moms`](@@@) shiny App allows exploration the effect of many more metrics as well as allowing customization of metrics by combining them or entering user-designed functions.
...
```{r fig_reduce_space, echo = FALSE, fig.height = 12, fig.width = 8, results = 'hide', fig.cap = paste("Figure 3: different type of space reduction. Each panel displays two groups (orange and blue) of 50% of the data points each. Each group are generated using the following algorithm: A - randomly; B - by limit (maximum and minimum limit in respectively orange and blue); C - by density (high and low in respectively orange and blue); and D - by position (positive and negative in respectively orange and blue). Panel E represents a typical display of the reduction results displayed in Table 6@@@: the dots represent the median space occupancy values across all simulations for each scenario of trait-space change, the solid and dashed line respectively the 50\% and 95\% confidence intervals. Results in grey are the random 50\% reduction (panel A). Results in blue and orange represent the blue and orange opposite scenarios from panels B, and D. The displayed numerical value is the probability of overlap (Bhattacharrya Coefficient) between the blue and orange distributions and the grey one.")}
...
par(bty = "n")
## Plot size
plot(NULL, ylim = c(0.8, 3.2), pch = 19, xlim = c(-1,1), xlab = "Centred and scaled space occupancy", ylab = "", xaxt = "n", yaxt = "n", main = "E - Space reduction results example (Table 6)")
## Adding lines
...
Furthermore, high dimensional spaces (>10) are subject to the "curse of multidimensionality" [@cursedimensionality] which makes them "behave"
<!-- MP: I think this clarification – I could not think of a good way though…Alternatively leave it out and go on to the explanation in the next sentence. -->
 differently than their counterparts with a lower number of dimensions.
...
We then divided the trait-spaces into two biologically-relevant groups and tested whether the different metrics differentiated the groups in different ways.
<!-- AM: I hope I didn’t cut out meaning  -->
...
<!-- AM: Way to say this more strongly? Choosing the right combination of metrics to answer a specific biological question -->
...
We also show that the choice of metric impacts on the interpretation of group differences in empirical datasets (Table 7@@@), again emphasizing that metric choice has a real impact on 
<!-- AM: interpretations of(?) -->
...
Furthermore,
<!-- AM: when XY and Z is done,  -->
 all metrics seem to behave following a normal distribution except for the ellipsoid volume which demonstrates a classic "curse of multidimensionality" with values increasing logarithmically first and then quickly decreasing to 0 [Fig. 4@@@; @cursedimensionality].
Overall, the fact that we have such a range of correlations for such conserved 
<!-- AM: I didn’t get this part of the sentence. Is it because they all (almost) have normal distributions? -->
distributions suggests that each metric can capture different summaries of space occupancy ranging from obvious differences (for metrics not strongly correlated) to subtle ones (for metrics strongly correlated).
<!-- AM: This is super cool and if you can, I’d recommend highlighting this more -->
...
<!-- AM: I think this should be highlighted more as this is the reason you’ve created moms.  -->
...
<!-- However, the shift from a small volume (orange; Fig. 3@@@-B) to a bigger "hollow" volume (blue; Fig. 3@@@-B) can still be interpreted has an increase in volume. -->
...
<!-- AM: Even more so in the trilobite data… -->
...
We insist that there is no metric better than another
<!-- AM: Does this mean, keep running more metrics? Perhaps “no metric better than the next one”? How is this clause different from the first suggestion (sentence 3) -->
 one but that researchers should use the most appropriate metrics based on their question, the specific trait-space, and knowledge of the metric’s properties. 
...
<!-- AM: What is the distinction between choosing metrics for the trait-space versus metrics to answer the question? Should one be done first? -->
