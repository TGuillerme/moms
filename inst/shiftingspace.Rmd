...
<!-- VW: “Simulation of…” instead of “We did x”? -->
...
<!-- VW: Could you add here or elsewhere a hypothetical biological scenario where such trait space changes could occur?
Also, you talk about “displacement” here but “position” above. I suggest picking one.  -->
3.  We measured occupancy on the resulting transformed spaces using seven different space occupancy metrics;
...
| How are metrics affected by space shifts? | Space shifts              | Table 5
...
We then measured height @@@ different metrics on the resulting transformed spaces, including a new metric we produced:
 <!-- VW: [Justification] : -->
...
We selected these height @@@ space-occupancy
<!-- AM: I think this is a new term… is it equivalent to trait-space occupancy? (apply this question to all instances of the term) -->
 metrics to illustrate how they capture different aspects of trait-space occupancy (but note that this is not an expression of  our preference).
...
To measure how the metrics responded to shifts in 
<!-- AM: [how the dataset filled the] Is it correct to imagine a morphospace that is roughly where the data will be first and then think about how the dataset is filling it or shifting within it? In other words, does the trait-space really "shift” or is it the data doing that? Given the section heading… I’m assuming I’m wrong but that maybe a sentence or two about this would help folks thinking like me to disentangle what the data is doing from the [graphical] space itself.  -->
 trait-spaces, we reduced the spaces by removing 50% of elements each time using the following algorithms:
<!-- VW: This might be a good place for a biological rationale -->
...
<!-- AM: Worth it to acknowledge here which algorithms directly impact one of the 3 categories of metric?   -->
...
<!-- VW: Wanna throw in the wombat paper? -->

### Measuring the effect of space and dimensionality on shifting spaces

We also measured the potential effect the distribution of the space or the number of dimensions in the space could have on the metric in general. In fact, the difference of distributions can have a drastic effect on the way space-occupancy is measured.
<!-- VW: This could be clearer and I think these two sentences are redundant. Mayebe more like “in some situations, distribution differences can have an effect on the metric results. For example… We therefore also measure the potential effect… -->
...
The differences in distributions can also have serious effects in high dimensional spaces (>10) which are subject to the “curse of multidimensionality” [@cursedimensionality].
...
We measured the effect of space distribution and dimensionality using an ANOVA ($occupancy \sim distribution$ and $occupancy \sim dimensions$) by using 1)
<!-- VW: The numbers in this paragraph is confusing to me – is it necessary to number?  -->
 all spaces with 50 dimensions and 2) the uniform and normal spaces with equal variance and no correlation with 3, 15, 50, 100 and 150 dimensions (Table 2) for testing respectively the effect of 1) distribution and 2) dimensions.
The results of the ANOVAs (*p*-values) are reported in Table 3 (see supplementary material for the full ANOVA results).
...
We then divided the trait-space into two roughly equal sized groups based on
 <!-- VW: the biological question? Properties of the sample studied? -->
 and applied the space-occupancy metric to each pair of groups to test whether different metrics captured different differences between groups. 
<!-- VW: Is it worth adding how the questions were designed? I can see that some of them reflect the paper’s questions but the gopher question seems new, for example.  -->
...
@hopkins2016 | geometrics morphometrics | Trilobita | 3D landmark coordinates | Ordination of a Procrustes Superimposition (PCA) | 46*46 | 10 juveniles vs. 36 adults | Are juvenile trilobites a subset of adult ones? |
...
<!-- Figure 4: pairwise correlation between the scaled metrics. Each data point represents the measures for the metrics for each of the 260 simulated trait spaces. Numbers on the upper right corner are the Pearson correlations. The red line are linear regressions (with the confidence intervals in grey). -->
<!-- VW: correct? -->
...
All the metrics selected were either positively correlated (correlation
<!-- VW: Too complicated ! -->
 of 0.99 for the Average distance from centroid and Sum of variance or 0.97 for the Average nearest neighbour distance and Minimum spanning tree average length; Fig. 4) or somewhat correlated (ranging from 0.66 for the Sum of Variances and the Ellispsoid Volume to -0.09 between the Average displacement and the Average distance from centroid; Fig. 4).
...
Note however that the increase in volume does not correspond to an *actual* increase in volume in the trait-space (i.e. the volume in from the orange
<!-- VW: Shouldn’t it be blue? The volume of the blue observations is equivalent to the volume of the space in 3A? -->
 observations in Fig. 3B is equivalent to the one in Fig. 3A).
...
This is not surprising however, since the notion of positions becomes more and more complex to 
 <!-- VW: grasp? -->
 as dimensionality increases (i.e. beyond left/right and up/down).
...
<!-- VW: This needs more description. In particular, for example, The Beck and Lee data suggest that the KT boundary resulted in volume differences but not density or position difference; Hopson and Pearson suggest that red has a much larger volume than blue but blue is also displaced. Ideally in one or several of these we find an indication of a previously unfound piece of information.  -->
...
The correlation
<!-- VW: Could there be a comment about how correlated “related” metrics that target the same basic shift (volume, density, position as per Table 3) are? Eg. Distance from centroid and sum of variances is clearly nearly the same – but sum of ranges is also meant to reflect volume (per your description in the intro) but there’s a much weaker correlation. -->
 between metrics ranges from a near 1:1 correlation (0.99 for the mean distance from centroid and the sum of variances) to a somewhat negative correlation (-0.23 for the sum of ranges and the spanning tree average length).
Furthermore, all metrics seem to behave following a normal distribution, except for the ellipsoid volume which demonstrates a classic “curse of multidimensionality” 
<!-- AM: If we’re running with this (and I hope we are), definitely use the whole phrase back in the intro or methods when you introduce it -->
with values increasing logarithmically first and then quickly decreasing to 0 [Fig. 4; @cursedimensionality].
In other words, the ellipsoid volume is clearly sensitive to changes in trait-space only up to a modest number of dimensions.
The fact that we have such a range of correlations for such conserved distributions (but for the ellipsoid volume) suggests that each metric can capture different summaries of trait-space occupancy ranging from obvious differences (for metrics not strongly correlated) to subtle ones (for metrics strongly correlated).
Comparisons between more metrics are available in the Supplementary Material 4@@@.
<!-- VW: And what do they say? Same thing? -->
<!-- VW: This paragraph needs more discussion. -->

#### Space shifting

Most metrics capture a normal “null” space shift with no changes in space occupancy on average (i.e. randomly removing 50% of the data - in grey in Table 5).
...
This is not especially a bad thing for both metrics
<!-- VW: ? but only when we already know that the process is non-random, right? -->
 (i.e. they can still be used to analysis changes in space occupancy) but should be kept in mind as a property of these metrics.
...
This could be due to the fact that these metrics are merely approximating changes in volume with the volume itself
<!-- VW: I don’t understand this. But it seems like we can conclude that any “hollowing out” of trait space will not be captured but it could be a real problem. But I guess this not a biologically likely scenario… -->
 (e.g. the ellispsoid volume) being more sensitive to the curse of multidimensionality  [@cursedimensionality].

Regarding changes in density of the trait-spaces, the average nearest neigbhour distance and the minimum spanning tree average distance consistently detect changes in density with more precision for low density trait-spaces (in blue in Table 5).
...
<!-- VW: So in real life it is unlikely to be a problem? -->

Regarding the changes in position of the trait-space, all but the average displacements metric seems to not be able to distinguish between a random change in trait-space (grey) and a positive or a negative displacement of the trait-space (blue and orange - Table 5).
...
<!-- VW: I am not sure if this is surprising – I mean, you can have a smaller volume in one group but a higher density in the other?  -->
...
<!-- VW: I’d rephrase this because it puts undue emphasis on this issue. I know people fight over the best metric but it seems enough to just to summarize which finds of the study are prompting our suggestions and then move on to suggestions themselved. -->
...
Third, we suggest to not name metrics as the biological aspect they are potentially describing (e.g. disparity
<!-- VW: This may need to be raised as an issue in the intro -->
 or functional dispersion) but rather what they are measuring (e.g. sum of dimensions variance or the average distance from centroids).
