...
## The list of volume density metrics
position_metrics <- list("med.eucl.dist.centre" = list(c(median, centroids), centroid = 0),
                        "sum.eucl.dist.centre" = list(c(sum, centroids), centroid = 0),
...
volumes_300e_10d <- simulate.metrics(20, elements, dimensions, distributions, remove, volume_metrics, verbose = TRUE)
density_300e_10d <- simulate.metrics(20, elements, dimensions, distributions, remove, density_metrics, verbose = TRUE)
positio_300e_10d <- simulate.metrics(20, elements, dimensions, distributions, remove, position_metrics, verbose = TRUE)

## Summarising the data
summar_vol <- summarise.metrics(volumes_300e_10d)
summar_den <- summarise.metrics(density_300e_10d)
summar_pos <- summarise.metrics(positio_300e_10d)
```

```{r fig_volume_results, echo = FALSE, fig.height = 10, fig.width = 8}
results <- summar_vol
## Plotting the results
plot.metrics(results[[1]], results[[2]], col = default.palette(length(results[[1]])),
             space = typical_removal[[1]], remove_list = typical_removal[[2]], defaults)
```

```{r fig_density_results, echo = FALSE, fig.height = 10, fig.width = 8}
results <- summar_den
## Plotting the results
plot.metrics(results[[1]], results[[2]], col = default.palette(length(results[[1]])),
             space = typical_removal[[1]], remove_list = typical_removal[[2]], defaults)
...
```{r fig_position_results, echo = FALSE, fig.height = 10, fig.width = 8}
results <- summar_pos
## Plotting the results
plot.metrics(results[[1]], results[[2]], col = default.palette(length(summar_vol[[1]])),
             space = typical_removal[[1]], remove_list = typical_removal[[2]], defaults)
```


```{r, eval = FALSE, echo = FALSE}
## Are the reductions working?
defaults$xlim <- defaults$ylim <- NULL
trait_space <- space.maker(elements, 3, distribution = runif)

## The randomly removed space
limit_rm <- reduce.space(trait_space[], type = "limit", remove = 0.5)
## Plotting the reduction
plot.space(trait_space, limit_rm, main = "", defaults, axis = c(1,2))
plot.space(trait_space, limit_rm, main = "", defaults, axis = c(1,3))
plot.space(trait_space, limit_rm, main = "", defaults, axis = c(2,3))

plot.space(trait_space, limit_rm, main = "", defaults, axis = c(3,4))
