...
For example @wright2017 uses the volume of trait space to analyse how crinoid ecology changes through time relative to all the observed ecologies (i.e. the cronoid trait-space).
...
Although density is an aspect of space occupancy is measured less often than the volume, it is still used in both evolution [e.g. the average pairwise distance; @geiger2008] and in ecology [e.g. the minimum spanning tree length; @oksanen2007vegan].
...
For example, when looking at the effect of mass extinction on space occupancy, a change in volume (or the absence thereof) can be decoupled from a change in position or density of the observations trait-space.
...
This will allow resarchers to comprehensively assess the properties of their traitspace and metrics associated with their specific biological question.
