...
> @@@TG: Add a reference to these works.
...
> @@@TG: we agree with the reviewer and have toned down or "diplomacy" level. We now also state concern about certain types of metrics that are highly sensitive to outliers or to the number of dimensions.
...
> @@@TG: We believe this is a really interesting point that we didn't covered in the previous version of the manuscript. However we did not really found a way to fit in as a point in the main text. We've rerun part of the analysis (reproducibility made that easy!) and added a novel supplementary material covering the effect of sampling in details.
...
> We already specified that "the dots represent the median space occupancy values across all simulations for each scenario of trait space change (Table 2), the solid and dashed line respectively the 50% and 95% confidence intervals" (Caption Figure 2). We've now also added it to the Table 5 caption.
...
> @@@ TG: fix this throughout the manuscript
Nice spotting, this comes from a classic version mashup. We've fixed this throughout the manuscript.
