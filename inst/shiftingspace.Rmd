...
<!-- > MNP. Rather than just observing differences, may it be more important to

    (i) show what different metrics measure and can we actually relate this to underlying biology, and

    (ii) generate null changes with parameters of interest for a researcher's clade.
...
Metric     | Beck and Lee 2014  | Wright 2017 | Marcy et al. 2016 | Hopkins et al. 2016 | Jones et al. 2015 | Healy et al. 2019  |
:-----------|----------------|-----------------|------------------|----------------|------------------|----------------|
...
Expected "null" change in directionality: Average nearest neighbour distance (positive median) and sum of ranges (negative median).
This means that for any decrease in number of elements in the trait-space, the average nearest neighbour distance and the sum of ranges will always pick up respectively and increase and a decrease in trait-space occupancy.
This is not especially a bad thing but definitely something to keep in mind.

Change in volume:
Sum of variances and average distance from centroid really good since there is only a low overlap.
This is good since they pick something different than the simple random decrease.
However they also pick some increase in volume (blue) even though the algorithm does only suppress data from the centre (Fig. 3B) which does not correspond as an increase in volume *per se*.
Furthermore they also seem to be correlated to changes in density

Change in density:
Average nearest neigbhour distance and minimum spanning tree average distance seems to be the best.
However both seem to have a "null increase" (see below).

Change in position:
Unsurprisingly changes in position is hard to capture (position gets more complex as dimensionality increases).
The average displacement seems to be the best at picking a change in position and maybe the sum of ranges but with a big "null decrease" (leading to high Bhattacharrya Coefficients - 0.96 and 0.86 - Tab. 5).

#### Empirical examples

Again, as expected, different metrics highlight different differences for different trait-spaces.
Although most differences are fairly consistent within each dataset (red group mean superior/inferior to blue group mean), this can be more or less pronounced (ranging from no to nearly full overlap - BC $\in(0;0.995)$) 

- Metrics results depends on the space
- However some metrics always increase/decrease (e.g. @@@) even when removing points randomly

- This shows that the choice of the metric is a really important step and should be carefully thought through depending on what to measure.
For example if we want to know whether there is a displacement in the trait-space, using @@@ metrics will always result in false negative results.
Similarly, if we want to know a change in position, using @@@ will often result in false positives.
...
We insist that there is no metric better than another one and that people should use their metrics based on their question and knowledge of both the metric and the trait-space properties.
However, following this study we suggest the following points could be applied to future multdimensional analysis:
...
* Finally we suggest checking the caveats of the metric used (e.g. curse of multidimensionality)

We suggest using the [`moms`](@@@) shiny app to help exploring this.
