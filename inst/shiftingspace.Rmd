...
Our results show that the correlation between metric results can vary both within and between metric categories (Fig. 4@@@), highlighting the importance of understanding the metric classification for the interpretation of results
...
We also show that the choice of metric impacts on the interpretation of group differences in empirical datasets (Table 7@@@), again emphasizing that metric choice has a real impact on specific biological questions.
...
Overall, the fact that we have such a range of correlations for such conserved distributions suggests that each metric can capture different summaries of space occupancy ranging from obvious differences (for metrics not strongly correlated) to subtle ones (for metrics strongly correlated).
...
We believe this point will allow both a clearer understanding of what is measured (c.f. what is approximated) and a easier understanding between ecology and evolution for which the metrics can be the similar but named differently and used for approximating different types of trait occupancy (Fig. 4@@@).

Multidimensional analysis have been acknowledged to be an essential toolkit modern biology [@adams2019] but can often be counter-intuitive [@cursedimensionality].
...
TG, MNP, AM and VW designed the project.
TG and AM collected the empirical dataset.
TG ran the analyses and designed the software.
TG, MNP, AM and VW wrote the manuscript.
