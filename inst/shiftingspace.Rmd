...
```{r fable_metrics, fig.show='hide', echo= FALSE}
library(knitr)
...
metric_names <- c("Ellipsoid volume", "Average squared distance", "Procrustes variance", "Product of Range", "Sum of Ranges", "Product of Variances", "Sum of Variances", "Product of Quantiles", "Sum of Quantiles", "Mean displacement", "Mean nearest neighbours distance")
```
...
## Plotting all the results as individual plot (for the table)
elements <- 300
## The amount to remove
remove <- 0.5
...
```{r, echo = FALSE}
## Helper functions for plotting the mini-plots in the table
plot.id <- function(plot.ID, path = "fig.path", chain = "fable_results", resolution = 800) {
    knitr::include_graphics(path = paste0(opts_chunk$get(path), paste0(chain, "-", plot.ID, ".png")),
                            dpi = resolution)
}

## Helper function for summarising the tests
s.test <- function(metric.ID, type, test.results = list("s" = test_space05, "r" = test_raref05)) { 
    ...
    ## Display the results
    output <- paste(c(
              paste(c("", colnames(print_results)), collapse = "\t"),
              paste(c(rownames(print_results)[1], unname(print_results[1,])), collapse = "\t"),
              paste(c(rownames(print_results)[2], unname(print_results[2,])), collapse = "\t")
              , collapse = "\n"))
...
Metric | Random change | Volume change | Position change | Density change | Distribution effect | Dimensions effect
:------|---------------|---------------|-----------------|----------------|---------------------|-------------------
metric1|`r plot.id(1)` |`r plot.id(2)` |`r plot.id(2)`   |`r plot.id(2)`  | `r s.test(1, "s")`  | `r s.test(1, "r")`
metric2|`r plot.id(1)` |`r plot.id(2)` |`r plot.id(2)`   |`r plot.id(2)`  | `r cat(1)`  | 1  
