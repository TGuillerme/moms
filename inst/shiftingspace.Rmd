...
However, different fields use a different set of terms and approaches for such trait-space approaches (Table `r tables <- table("glossary", add = TRUE); tables$print`).
...
Table `r tables <- table("glossary"); tables$print`: terms and equivalence between mathematics, ecology and macroevolution
...
However, regardless of the methodology used to build a trait-space, three main broad occupancy metrics can be measured to investigate how observations are distributed in the trait-space: the volume which will approximate the amount of space occupied, the density which will approximate the distribution in space and the position which will approximate the location in space [Fig. `r figures <- figure("differences", add = TRUE); figures$print`; @villéger2008].
...
Figure `r figures <- figure("quartets", add = TRUE); figures$print` illustrates this problem on a 2D trait space with eight hypothetical observation where the three aspects (volume, density and position) can change in concert or independently.
...
```{r, echo = FALSE}
## Skip one table count for the figure comparisons
figures <- figure("reductions", add = TRUE)
```

| Test | Methods section | Results
|------|-----------------|------------------
| How do the metrics compare to each other? | Metrics comparisons       | Figure `r figures <- figure("comparisons", add = TRUE); figures$print`
| How are metrics affected by space shifts? | Space shifts              | Table `r tables <- table("simulations", add = TRUE); tables$print`
| How is that reflected in the real world?  | Empirical examples        | Table `r tables <- table("empiricals", add = TRUE); tables$print`
Table `r tables <- table("tests", add = TRUE); tables$print`: summary of the tests in this paper.
...
Table `r tables <- table("spaces", add = TRUE); tables$print`: different simulated space distributions.
...
* **Randomly:** by randomly removing 50% of elements (Fig. `r figures <- figure("reductions"); figures$print`A).
...
* **Limit:** by removing all elements with a distance from the centre (mean point) of the space lower or greater than a radius $\rho$ (where \$rho$ is selected such that 50% elements are selected) generating two limit removals: *maximum* and *minimum* (respectively in orange and blue; Fig. `r figures <- figure("reductions"); figures$print`B).
...
* **Density:** by removing any pairs of point with a distance $D$ from each other where (where $D$ is selected such that 50% elements are selected) generating two density removals: *high* and *low* (respectively in orange and blue; Fig. `r figures <- figure("reductions"); figures$print`C).
...
* **Position:** by removing points similarly as for **Limit** but using the distance from the furthest point from the centre (max point) generating two position removals: *positive* and *negative* (respectively in orange and blue; Fig. `r figures <- figure("reductions"); figures$print`D).
...
This "curse" can make the interpretation of high dimensional data counter-intuitive.
For example if a group expands in multiple dimensions (i.e. increase in volume), the actual hypervolume can decrease (Fig. `r figures <- figure("comparisons"); figures$print` and Tables `r tables <- table("simulations"); tables$print`, `r tables <- table("empiricals"); tables$print`).

We measured the effect of space distribution and dimensionality using an ANOVA ($occupancy \sim distribution$ and $occupancy \sim dimensions$) by using all spaces with 50 dimensions and the uniform and normal spaces with equal variance and no correlation with 3, 15, 50, 100 and 150 dimensions (Table `r tables <- table("spaces"); tables$print`) for testing respectively the effect of distribution and dimensions.
The results of the ANOVAs (*p*-values) are reported in Table `r tables <- table("simulations"); tables$print` (see supplementary material for the full ANOVA results).
...
Table `r tables <- table("datasets", add = TRUE); tables$print`: details of the six empirical trait-spaces.
...
The are displayed in Table `r tables <- table("empiricals"); tables$print`.
...
<!-- VW: Too complicated ! -->
 of 0.99 for the Average distance from centroid and Sum of variance or 0.97 for the Average nearest neighbour distance and Minimum spanning tree average length; Fig. `r figures <- figure("comparisons"); figures$print`) or somewhat correlated (ranging from 0.66 for the Sum of Variances and the Ellispsoid Volume to -0.09 between the Average displacement and the Average distance from centroid; Fig. `r figures <- figure("comparisons"); figures$print`).
Note that all metrics but the Ellipsoid Volume were normally (or nearly normally) distributed (Fig. `r figures <- figure("comparisons"); figures$print`).
...
Table `r tables <- table("simulations"); tables$print`: Results of the effect of space reduction, space dimension distributions and dimensions number of the different trait-space occupancy metrics. The dots represent the median trait-space occupancy values across all simulations for each scenario of trait space change, the solid and dashed line respectively the 50% and 95% confidence intervals. Results in grey are the random 50% reduction. Results in blue and orange represent the blue and orange opposite scenarios in Fig. `r figures <- figure("reductions"); figures$print` (blue,  the minimum limit, low density and negative shift orange, maximum limit, high density and positive shift) for respectively the volume, density and position changes.
...
As expected (Fig. `r figures <- figure("quartets"); figures$print`), some different metrics capture different aspects of trait-space occupancy.
In contrast to the clear hypothetical example in Fig. `r figures <- figure("quartets"); figures$print`, it can be hard to predict the behaviour of each metric when 50% of the observations are removed.
...
<!-- VW: Shouldn't it be blue? The volume of the blue observations is equivalent to the volume of the space in 3A? -->
 observations in Fig. `r figures <- figure("reductions"); figures$print`B is equivalent to the one in Fig. `r figures <- figure("reductions"); figures$print`A).
...
Table `r tables <- table("empiricals"); tables$print`: Comparisons of different pairs of groups in different empirical trait-spaces. NAs are used for cases where trait-space occupancy could not be measured due to the curse of multidimensionality. The displayed values are the probability of overlap between both groups (Bhattacharrya Coefficient).

Similarly as for the simulated results, the empirical ones indicate that there is no perfect one-size-fit all metric.
For all `r metric.names()` metrics (expect the Ellispoid volume) we see either one group or the other having a bigger mean than the other.
...
As expected we see a range of differences between the metrics themselves (Fig. `r figures <- figure("comparisons"); figures$print`), their response to space shifting (Table `r tables <- table("simulations"); tables$print`) and their effect on capturing differences in empirical datasets (Table `r tables <- table("empiricals"); tables$print`).
...
with values increasing logarithmically first and then quickly decreasing to 0 [Fig. `r figures <- figure("comparisons"); figures$print`; @cursedimensionality].
...
Most metrics capture a normal "null" space shift with no changes in space occupancy on average (i.e. randomly removing 50% of the data - in grey in Table `r tables <- table("simulations"); tables$print`).
...
However, the average nearest neighbour distance and the sum of ranges have a respectively positive and negative "null" median meaning that any decrease of elements will always lead to respectively and increase and a decrease of these metrics even when the process is random (Table `r tables <- table("simulations"); tables$print`).
...
However, as illustrated in the 2D examples in Fig. `r figures <- figure("reductions"); figures$print`B, only the minimum limit changes (orange) should actually lead to a decrease of volume of the trait-space; the volume of the maximum limit shifted trait-spaces should have the same volume as the non-modified trait-space but with a hole in its centre.
...
Regarding changes in density of the trait-spaces, the average nearest neigbhour distance and the minimum spanning tree average distance consistently detect changes in density with more precision for low density trait-spaces (in blue in Table `r tables <- table("simulations"); tables$print`).
...
Regarding the changes in position of the trait-space, all but the average displacements metric seems to not be able to distinguish between a random change in trait-space (grey) and a positive or a negative displacement of the trait-space (blue and orange - Table `r tables <- table("simulations"); tables$print`).
...
For example, in @wright2017 dataset where the blue group has a greater occupancy metric median value but for the average displacements metric showing a higher median displacement in the red group (Table `r tables <- table("empiricals"); tables$print`).
