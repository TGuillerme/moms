...
<!-- Add that @villéger2008 notes three distinctions: volume (FRic Fig.1b); minimum tree spanning (MST Fig.1c); -->
...
|------|-----------------|------------------
| How do the metrics compare to each other? | Metrics comparisons       | FigX
| How are they affected by space shifting?  | Space shifting            | FigX+1
| How is that reflected in the real world?  | Empirical examples        | FigX+2


<!-- | How are they affected by dimensionality?  | Effect of dimensionality  | FigX+1 -->
...
|------------|-------|--------------------------------|---------------------|-------------|
| Uniform3   |200*3  | Uniform (min = -0.5, max = 0.5)| Constant            | none        |
| Uniform15  |200*15 | Uniform                        | Constant            | none        |
...
| Normal3    |200*3  | Normal  (mean = 0, sd = 1)     | Constant            | none        |
| Normal15   |200*15 | Normal                         | Constant            | none        |
...
Median distance from centroid[^1] |  median($\sqrt{\sum_{i}^{n}{({k}_{n}-Centroid_{k})^2}}$) | Volume and density | | | |
...
[^1]: 
This is equivalent to the functional diversity index.



<!-- TODO: add a series of metrics corresponding to the ecology metrics? -->
...
metrics_list <- list(#"av.pairwise" = function(matrix) return(mean(pairwise.dist(matrix)^2)),
                    ...
                    "dist.centre" = list(c(mean, centroids), centroid = 0),
                    "mean.centr" = c(mean, centroids),
                    "func.disp"  = function(matrix) return(unname(FD::fdisp(dist(matrix))$FDis)),
                    "func.div"  = func.eve,
                    "func.eve"  = func.div,
                    "conv.hull" = function(matrix) return(geometry::convhulln(matrix, "FA")$vol),
...
All metrics are scaled between 0 and 1 for comparisons.
<!--  -->
<!--  -->
<!--  -->
<!-- COMPARING METRICS -->
...
plot.space(trait_space, displace_rm, main = "D - Displacement", defaults)
...
## What is the effect of changing the space?
...
```{r fig_metric_correlation, echo = FALSE, fig.height = 6, fig.width = 6, results = 'hide', fig.cap = paste("Figure @@@: correlation between the scaled metrics. Numbers on the upper right corner are the Pearson correlations. The red line are linear regressions (with the confidence intervals in grey).")}
...
```{r fig_metric_dimensionality, eval=FALSE, echo = FALSE, fig.height = 6, fig.width = 6, results = 'hide', fig.cap = paste("Figure @@@: correlation between the scaled metrics. Numbers on the upper right corner are the Pearson correlations. The red line are linear regressions (with the confidence intervals in grey).")}
...
```{r fable_results, fig.show='hide', echo=FALSE, fig.height=3.5, fig.width=3.5}
...
## List of spaces
trait_spaces <- c(palaeo, gmm, ecol)
...
```{r fable_results_empirical, fig.show='hide', echo=FALSE, fig.height=3.5, fig.width=3.5}
## Plotting parameters
...
:-----------|----------------|-----------------|------------------|----------------|------------------|----------------|
`r name[1]` | `r plot.id(1)` | `r plot.id(11)` | `r plot.id(21)` | `r plot.id(31)` | `r plot.id(41)` | `r plot.id(51)`|
`r name[2]` | `r plot.id(2)` | `r plot.id(12)` | `r plot.id(22)` | `r plot.id(32)` | `r plot.id(42)` | `r plot.id(52)`|
`r name[3]` | `r plot.id(3)` | `r plot.id(13)` | `r plot.id(23)` | `r plot.id(33)` | `r plot.id(43)` | `r plot.id(53)`|
`r name[4]` | `r plot.id(4)` | `r plot.id(14)` | `r plot.id(24)` | `r plot.id(34)` | `r plot.id(44)` | `r plot.id(54)`|
`r name[5]` | `r plot.id(5)` | `r plot.id(15)` | `r plot.id(25)` | `r plot.id(35)` | `r plot.id(45)` | `r plot.id(55)`|
`r name[6]` | `r plot.id(6)` | `r plot.id(16)` | `r plot.id(26)` | `r plot.id(36)` | `r plot.id(46)` | `r plot.id(56)`|
`r name[7]` | `r plot.id(7)` | `r plot.id(17)` | `r plot.id(27)` | `r plot.id(37)` | `r plot.id(47)` | `r plot.id(57)`|
`r name[8]` | `r plot.id(8)` | `r plot.id(18)` | `r plot.id(28)` | `r plot.id(38)` | `r plot.id(48)` | `r plot.id(58)`|
`r name[9]` | `r plot.id(9)` | `r plot.id(19)` | `r plot.id(29)` | `r plot.id(39)` | `r plot.id(49)` | `r plot.id(59)`|
`r name[10]` | `r plot.id(10)` | `r plot.id(20)` | `r plot.id(30)` | `r plot.id(40)` | `r plot.id(50)` | `r plot.id(60)`|
