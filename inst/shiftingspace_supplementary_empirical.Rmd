...
## Comments / questions from Ariel (delete when resolved :)
1) Should there be a blank line before new segments, e.g. line 29 "## Setting the defaul parameters for the space plots" ?
2) I don't really understand what is going on with the markdown code in line 54 but seems to be something with citations? On line 55 one of the lagging asterisks is white instead of red (which says to me the formatting isn't being picked up?)
3) Is "Claddis.ordination()" the function that creates the distance matrix from line 54? If I were researcher, I would like that to be commented in.
4) Oh no! The moms package didn't load!
