...
jones_communities <- read.csv("../data/raw/Jones2015.csv")
...
<!-- ### Healy et al. 2019 data


-We log10 all variables and z-transformed by mean centering and dividing by the standard deviation prior to analysis

For each of the life history traits included used the residuals from the respective regressions, as described above, where we controlled for variation relating to mass, matrix dimension, phylogeny and population.

Life history trait ~ fixed = log10(mass) + log10(matrix dimension)
...
# t.test.safe <- function(x, y) {
#     if(length(x) < 2 || length(y) < 2) {
#         null_test <- t.test(c(1,2), c(1,2))
#         null_test$statistic <- c(t = NA)
#         null_test$p.value <- NA
#         return(null_test)
#     } else {
#         return(t.test(x, y))
#     }
...
disparity_t_test <- lapply(disparities, lapply, test.dispRity, test = t.test)#.safe)


bob <- lapply(disparities[[6]], test.dispRity, test = t.test.safe)


test.dispRity(disparities[[5]][[1]], test = t.test.safe)



## Results list
empirical_results <- list("disparities", "disparity_t_test")
