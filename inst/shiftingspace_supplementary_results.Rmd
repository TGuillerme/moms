...
all_dim_test <- test.simulation(remove_05, test = anova.fun, scale = TRUE,
                                factors = c("uniform3", "uniform15", "uniform50","uniform150",
                                            "normal3", "normal15", "normal50","normal150"))
...
dim_norm_test <- test.simulation(remove_05, test = anova.fun, scale = TRUE,
                                 factors = c("normal3", "normal15", "normal50","normal150"))
...
convert.to.table <- function(results, factor) {
    ...
    output <- matrix("", nrow = length(results)*2, ncol = dim(one_result[[1]])[2]+2,
                     dimnames = list(c(), c("metric", "", colnames(one_result[[1]]))))
    ...
    for(one_table in seq(from = 1, to = length(results)*2, by = 2)) {
        as.matrix(results[[print_seq[one_table]]][[1]]) -> 
            output[one_table:(one_table+1), (1:dim(one_result[[1]])[2])+2] 
    }
