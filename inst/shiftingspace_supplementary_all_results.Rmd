...
all_metrics <- list("av.pairwise" = function(matrix) return(mean(pairwise.dist(matrix)^2, na.rm = TRUE)),
                    ...
                    "med.center"  = function(matrix) return(median(centroids(matrix, centroid = 0, method = "manhattan")))
                    )
...
The metrics analysed are the `r paste(all_metrics_names, collapse = ", ")`.
...
```{r generate_spaces_simulation, echo = FALSE, warning = FALSE, results = 'hide', eval = TRUE}
...
```{r shift_spaces_simulation, echo = FALSE, warning = FALSE, results = 'hide', eval = TRUE}
## Shifting each space
...
```{r get_disparity_simulation, echo = FALSE, warning = FALSE, results = 'hide', eval = TRUE}
## Measuring disparity
...
```{r running_empirical, echo = FALSE, eval = TRUE}
data(demo_data)
## Loading the data
...
```{r}
## Load all the data
load.results("all_metrics_remove_02")
load.results("all_metrics_remove_05")
load.results("all_metrics_remove_08")
load.results("all_metrics_empirical_results")
```


## Metrics pairwise comparisons

```{r}
## Adding proper names
results_pairwise_02 <- all_metrics_remove_02
results_pairwise_05 <- all_metrics_remove_05
results_pairwise_08 <- all_metrics_remove_08

## Numbering the metrics
seq(1: length(all_metrics_names)) ->
  names(results_pairwise_02[[1]]) ->
  names(results_pairwise_05[[1]]) ->
  names(results_pairwise_08[[1]])  

## Plotting the pairwise results
pairwise.plot(results_pairwise_02, scale = TRUE, type = "base", plot = "cor", main = "Results for 20% removal")
pairwise.plot(results_pairwise_05, scale = TRUE, type = "base", plot = "cor", main = "Results for 50% removal")
pairwise.plot(results_pairwise_08, scale = TRUE, type = "base", plot = "cor", main = "Results for 80% removal")
```

The metrics analysed are the `r paste(paste(seq(1:length(all_metrics_names)), all_metrics_names, sep = ": "), collapse = "; ")` (diagonally).




## Space shifting all metrics
...
### All metrics results 20% removal
...
### All metrics results 50% removal
