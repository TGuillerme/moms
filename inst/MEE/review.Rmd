...
>@@@ Insist that we don't have a "magic bullet" metric (what was on line 281???)
We've added a sixth panel to the Figure 2 were the two last panels shows an example of an ideal and bad metric to make it easier to understand the Table 5 results.
>@@@ Improve figure with space changes (with ideal/worst scenarios).
>@@@ Simplify simulation fable by only displaying volume for "volume" metrics etc.? Maybe not since we want to highlight that one metric can capture different things.
...
>@@@ Add a disclaimer that this works for Euclidean isotropic spaces and that different metrics should be used if these are not met.
