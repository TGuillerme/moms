...
*We insist that no measure is better than the next one and that researchers should identify the most appropriate measures based on their trait space properties as well as their specific biological question.* lines @@@
...
*Figure 2: this figure illustrates the different type of space reduction and how this could affect the measures for the simulation results displayed in table 5.* lines @@@
...
<p style="color:blue">I thus suggest a priori defining each metric as a volume, density or position metric and showing only the tests for each in the main text (alongside the ideal plot for a metric that captures that feature well).</p>
...
This definition excludes whether spaces are are truly "occupiable" (e.g. are there limits to the space, are some regions of the space inapplicable, etc.) which might be of importance in particulare spaces (e.g. in tree spaces; Hillis et al. 2005).* lines@ 70-74
...
*are groups occupying the same amount of trait space? Do some groups have contain more species than others in the same amount of trait space? Are some specific factors correlated to different patterns of occupancy of the trait space (e.g. are some factors correlated with more occupancy)?* lines@ 45-47.
...
*Note that density based metrics can be sensitive to sampling (e.g. in macroevolution, when only living species are present)*. lines@ 115-116
 ...
 *In this paper, we define trait spaces as any matrix where rows are observations and columns are traits, where both observations and traits are structurally related (i.e. the values in the columns are dependent of the rows - the observations).* lines@ 83-85

 > as well as in Table 1:

 *with a structural relation between rows and columns (e.g. $d$ are measurements for each $n$)* lines@@@
