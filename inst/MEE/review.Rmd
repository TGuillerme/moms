...
>@@@VW: The associate editor makes some other points that I suspect need adressing. For example [...complete rewrite is required. [...] The motivation and goals of the measures must be clearly laid out and evaluated.[...]. I would say that this needs some kind of response, particularly because there was no complete re-write. Even if it is just "the mathematical terms have ben clarified thanks to reviewer2, and we also took care to bla bla so the motivations and goals of the ms were clearer". For reasons X, Y,Z, we believe that the narrative of the manuscript is now improved. Although I don't get the comment at all. This is an utterly clear manuscript...
...
<p style="color:blue">I thus suggest a priori defining each metric as a volume, density or position metric 

>@@@VW Isn't that in Table 3? Or am I confusing something? If so, make the point that this was done.

<p style="color:blue">and showing only the tests for each in the main text (alongside the ideal plot for a metric that captures that feature well). 
